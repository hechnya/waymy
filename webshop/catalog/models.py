...
class FeauturedProductManager(models.Manager):
    def get_query_set(self):
        return super(
            FeauturedProductManager, self).get_query_set().filter(
                is_featured=True)
...
class BestsellerProductManager(models.Manager):
    def get_query_set(self):
        return super(
            BestsellerProductManager, self).get_query_set().filter(
                is_bestseller=True)
...
class AquaProductManager(models.Manager):
    def get_query_set(self):
        return super(
            AquaProductManager, self).get_query_set().filter(is_aqua=True)


class NewProductManager(models.Manager):
    def get_query_set(self):
        return super(
            NewProductManager, self).get_query_set().filter(is_new=True)
...
class Category(MPTTModel):
    ...
    parent = TreeForeignKey(
        'self', verbose_name=_(u'Parent category'),
...
class ProductImage(models.Model):
    ...
    product = models.ForeignKey(
        Product, verbose_name=u'Product', help_text=u'Referenced product')
...
class ProductVolume(models.Model):
    ...
    volume = models.DecimalField(
        max_digits=9, decimal_places=2, verbose_name=u'Объем',
        blank=True, null=True)

    weight = models.DecimalField(
        max_digits=9, decimal_places=2, verbose_name=u'Вес')

    price = models.DecimalField(
        max_digits=9, decimal_places=2, verbose_name=u'Цена')

    new_price = models.DecimalField(
        max_digits=9, decimal_places=2,
        blank=True, default=0.00, verbose_name=u'Новая цена')

    default = models.BooleanField(u'Основной набор', default=False)
    product = models.ForeignKey(
        Product, verbose_name=_(u'Product'), help_text=_(u'Referenced product'))
...
class GiftPrice(models.Model):
    ...
    image = models.ImageField(
        verbose_name=u'Фото подарка', upload_to='gifts/images/')
